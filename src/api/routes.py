...
def get_cart():
    try:
        ...
        cart = Cart.query.filter_by(
            user_id=user_id_int, is_active=True).first()  

        if not cart:
            cart = Cart(user_id=user_id_int)  
            db.session.add(cart)
            db.session.commit()
...
def get_orders():
    try:
        user_id_str = get_jwt_identity()  
        user_id_int = int(user_id_str)  
        orders = Order.query.filter_by(user_id=user_id_int).all()  
